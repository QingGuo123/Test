...
module.exports = {
    ...
    insertPrivateMessage: function() {
        var sql = "INSERT INTO PrivateMessage (senderid, receiverid, content, timestamp, location, status_code, isnew) VALUES ((SELECT id FROM User WHERE username = ?), (SELECT id FROM User WHERE username = ?), ?, ?, ?, (SELECT status_code FROM Status, User WHERE Status.senderid = User.id AND username = ?), ?);";
        return sql;
    },

    getPrivateMessageBySenderAndReceiver: function () {
        var sql = "SELECT content, timestamp, location, status_code, isnew FROM PrivateMessage, User WHERE username = ? AND User.id = PrivateMessage.senderid ORDER BY timestamp;";
        return sql;
    }
